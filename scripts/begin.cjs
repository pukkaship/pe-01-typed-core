...
// npm run begin
//
// Entry gate (LOCAL, honor-system). This is a scaffold that nudges you to think before
// you code — it is NOT enforced. The real, enforced gate is CI on your pull request
...
if (!/(hypothesis|error)/i.test(hypothesis)) {
  failures.push('hypothesis.md must mention "hypothesis" or "error" (shows you read docs/day1-micro-loop.md)');
}
...
console.log("\n\u2713 Ready. Now run: npm test  \u2014 one test is failing. Start there.\n");
