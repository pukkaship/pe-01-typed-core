...
//
// Bug gate (LOCAL, honor-system scaffold). Checks that the current bug's tests pass and
// its journal is filled in, then reveals the next failing test + journal template.
//
// This is NOT enforced — it is a learning scaffold that keeps you honest about doing one
// bug at a time. The enforced gate is CI on your PR. Sequence enforcement (you cannot
// skip ahead) is a paid-tier feature; on the free track, CI simply checks the final state.
