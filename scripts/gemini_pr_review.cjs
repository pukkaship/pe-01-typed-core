#!/usr/bin/env node
// Best-effort AI PR reviewer for Module 1. Milestone-aware: matches validate_pr.cjs
...
function highestUnlockedBug() {
  let n = 0;
  for (let i = 1; i <= 5; i++) {
    if (fs.existsSync(`src/__tests__/bug-0${i}.test.ts`) || fs.existsSync(`tests/test_bug_0${i}.py`)) {
      n = i;
    }
  }
  return n;
...
function buildMilestoneContext(currentBug) {
  const lines = [
    ...
    `**This checkout's milestone: Bug ${currentBug} of 5** (inferred from highest bug-0N.test.ts or test_bug_0N.py present).`,
    "",
...
const rules = fs.existsSync(".github/gemini-review-rules.md")
  ...
  : "Review this Module 1 PR for whether it explains WHY each fix was necessary.";
